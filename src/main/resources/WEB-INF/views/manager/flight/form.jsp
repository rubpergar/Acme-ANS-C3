...
<acme:form>
	...
	<acme:input-checkbox code="manager.flight.form.label.isUnpublished" path="isUnpublished"/>	
	<jstl:choose>	 
		<jstl:when test="${_command == 'show' && isUnpublished == false}">
			<acme:button code="manager.flight.legs" action="/manager/flight/list?masterId=${id}"/>
		</jstl:when>
		<jstl:when test="${acme:anyOf(_command, 'show|update|delete|publish')  && isUnpublished == true}">
